...
import { realClock } from './scheduler.js';

export const SELECTION_TTL = 30 * 60_000;
export const MAX_SELECTION_RECORDS = 1000;
export const MAX_SELECTION_BYTES = 256 * 1024;
export const selectionHash = key => createHash('sha256').update(key).digest('hex');

function validRecord(record) {
  return record && Object.keys(record).sort().join(',') === 'cityId,expiresAt,hash' &&
    /^[a-f\d]{64}$/.test(record.hash) &&
    (record.cityId === null || typeof record.cityId === 'string' && /^[1-9]\d{0,15}$/.test(record.cityId)) &&
    Number.isSafeInteger(record.expiresAt) && record.expiresAt > 0 && record.expiresAt <= 8_640_000_000_000_000;
}
...
export function createSelectionStore({ filePath = null, clock = realClock, logger = console } = {}) {
  ...
  function ready() {
    loading ||= (async () => {
      ...
      } catch (error) { if (error.code !== 'ENOENT') failed('cleanup', error); }
      try {
        ...
        if (saved?.version !== 1 || !Array.isArray(saved.records) || saved.records.length > MAX_SELECTION_RECORDS ||
        ...
        entries.clear();
        if (error.code !== 'ENOENT') failed('read', error);
      }
      scheduleExpiry();
    })();
    return loading;
