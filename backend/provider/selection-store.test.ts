...
import { createSelectionStore, MAX_SELECTION_BYTES, SELECTION_TTL, selectionHash } from './selection-store.ts';
import { manualClock, recordLog } from './test-helpers.ts';
...
async function setup(t: TestContext) {
  ...
  const options = { filePath, clock, logger: { error: (entry: unknown) => recordLog(logs, entry) } };
  const store = createSelectionStore(options);
  t.after(() => store.close());
  return { directory, filePath, clock, logs, options, store };
}

const record = (clock: ProviderClock, key = 'selection') => ({ hash: selectionHash(key), cityId: '3000015284', expiresAt: clock.now() + SELECTION_TTL });
...
test('record count stays at one thousand and unknown private fields are rejected', async t => {
  ...
  assert.ok(last);
  assert.deepEqual(await store.get(last.hash), last);
  const withPrivateField = { ...record(clock), offerId: 'private-offer' };
  await assert.rejects(store.remember([withPrivateField]), TypeError);
...
test('short filesystem reads still recover the entire bounded snapshot', async t => {
  ...
  const read: (this: FileHandle, buffer: Buffer, offset: number, length: number, position: number | null) => Promise<{ bytesRead: number; buffer: Buffer }> = prototype.read;
  await handle.close();
  t.mock.method(prototype, 'read', function (this: FileHandle, buffer: Buffer, offset: number, length: number, position: number | null) {
