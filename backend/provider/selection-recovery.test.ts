...
import type { ProviderAdapter, ProviderParameters, ProviderLogger } from './types.ts';
...
import { createMemoryStateStore } from './state.ts';
import { createSelectionStore, SELECTION_TTL } from './selection-store.ts';
...
async function setup(t: TestContext) {
  ...
  const controls: { discovery: string; link: string; quoteError: ProviderFailure | null } = { discovery: 'initial', link: 'valid', quoteError: null };
  const calls: { listings: number; details: ProviderParameters['hotelDetails'][] } = { listings: 0, details: [] };
  const logs: Record<string, unknown>[] = [];
  const logger: ProviderLogger = { info: entry => recordLog(logs, entry), error: entry => recordLog(logs, entry) };
  ...
  const adapter: ProviderAdapter = {
    ...
    async listingsPage({ context }) {
      ...
      rows[0].handoffUrl = controls.link === 'missing' ? null
        : originalOfferUrl({ context, offerId: rows[0].pclnId, cityId: String(rows[0].location.cityId) });
  ...
  const services: ReturnType<typeof createProviderService>[] = [];
  function start(selectionStore: SelectionStore = createSelectionStore({ filePath, clock, logger })) {
...
test('restart recovery keeps original pricing and handoff after price expiry without repeating changed discovery', async t => {
  ...
  assert.ok(recovered.offer.quoteExpiresAt);
...
test('recovery cache failures preserve live results and never disable the provider', async t => {
  ...
  const broken: SelectionStore = {
    async ready() { throw new Error('Unused readiness method'); },
...
test('a slow recovery write cannot overrun the search budget and remains held until it settles', async t => {
  ...
  const service = start({ async ready() { throw new Error('Unused readiness method'); }, async get() { return undefined; }, remember() { return writing; }, close() { return writing; } });
  const held = new Set();
  ...
  const closing = closeOperation.then(() => { closed = true; });
  await flush();
...
test('a slow recovery read keeps the detail deadline and ownership without starting discovery afterward', async t => {
  ...
  let finishRead!: (value?: SelectionRecord) => void;
  const reading = new Promise<SelectionRecord | undefined>(resolve => { finishRead = resolve; });
  const service = start({ async ready() { throw new Error('Unused readiness method'); }, get() { return reading; }, async remember() { return true; }, async close() { await reading; return true; } });
  const held = new Set();
