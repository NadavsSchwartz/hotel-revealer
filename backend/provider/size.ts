...
export function serializeBoundedJson(value: unknown, limitBytes = MAX_JSON_BYTES) {
  let bytes = 0;
  let first = true;
  ...
  const serialized = JSON.stringify(value, function (this: unknown, key: string, item: unknown) {
...
export function assertJsonSize(value: unknown, limitBytes = MAX_JSON_BYTES) {
