...
import { futureContext, listingRows } from './provider/test-helpers.ts';
import { createClientLimiter } from './admission.ts';
...
import { createMemoryStateStore } from './provider/state.ts';
import { ServiceError } from './provider/errors.ts';

type AppOptions = NonNullable<Parameters<typeof createApp>[0]>;
type HttpService = NonNullable<AppOptions['service']>;
interface RequestInput { method?: string; rawBody?: string; body?: unknown; partial?: boolean; headers?: http.OutgoingHttpHeaders }
interface HttpResponse<Body = ApiErrorResponse> {
...
function controlledService({ admit = false } = {}) {
  ...
  const hold = (_input: unknown, { admitUpstream }: RequestOptions = {}) => new Promise<unknown>(resolve => {
    if (admit) { assert.ok(admitUpstream); admitUpstream(); }
    calls.push({ resolve });
    for (const waiter of waiters) if (calls.length >= waiter.count) waiter.resolve();
  });
  const service: HttpService = {
    // @ts-expect-error Admission tests deliberately control arbitrary response bodies without provider fixtures.
    search: hold,
  ...
  return { calls, service,
    ...
    release(index: number, result: unknown = { offers: [] }) { calls[index].resolve(result); },
    releaseAll() { for (const call of calls) call.resolve({ offers: [] }); },
  };
}

async function serve(t: TestContext, controlled: { service: Partial<HttpService>; releaseAll(): void }, options: Omit<AppOptions, 'service'> = {}) {
  const app = createApp({ logger: null, service: { ...unusedService, ...controlled.service }, ...options });
  ...
  return {
    start<Body = ApiErrorResponse>(route = '/api/v1/hotelDeals', { method = 'POST', rawBody, body, partial = false, headers = {} }: RequestInput = {}) {
      ...
      const response = new Promise<HttpResponse<Body>>(resolve => {
        request = http.request({ hostname: '127.0.0.1', port: address.port, path: route, method,
          ...
          res.on('end', () => {
            ...
            resolve({ status: res.statusCode, headers: res.headers, text, body: body as Body | null });
          });
        });
        request.on('error', (error: NodeJS.ErrnoException) => resolve({ status: undefined, headers: {}, text: '', body: null, error: error.code }));
...
function assertBusy(response: HttpResponse) {
...
const caddyHeaders = (client: number) => ({ 'X-Hotel-Revealer-Client-IP': `203.0.113.${client}` });

test('completed timeout responses retain capacity until their owned shared work settles', { timeout: 5000 }, async t => {
  const releases: (() => void)[] = [];
  const service: Partial<HttpService> = {
    async search(_input, { holdWork } = {}) {
      assert.ok(holdWork);
      holdWork(new Promise<void>(resolve => releases.push(resolve)));
...
test('one client can hold four shared/cache-like operations while another can use the other four global slots', { timeout: 5000 }, async t => {
  ...
  const health = await app.start<{ provider: Awaited<ReturnType<ProviderService['status']>> }>('/health', { method: 'GET' }).response;
...
function assertClientBusy(response: HttpResponse, retryAt: number) {
  assert.ok(response.body);
  assert.equal(response.status, 503);
  assert.equal(response.body.error.code, 'PROVIDER_BUSY');
...
test('explicit Caddy boundary canonicalizes literal IPs and one exhausted client leaves capacity for another', { timeout: 5000 }, async t => {
  ...
  const start = (ip: string | string[]) => app.start('/api/v1/hotelDeals', { headers: {
...
test('bounded identity table never evicts active quotas and expired identities safely regain their full allowance', () => {
  ...
  const exhaust = (key: string) => { for (let index = 0; index < 4; index += 1) admit(key); };
  for (let index = 0; index < 1000; index += 1) exhaust(`client-${index}`);
  for (let index = 1000; index < 1050; index += 1) {
    assert.throws(() => admit(`client-${index}`), error => error instanceof ServiceError && error.code === 'PROVIDER_BUSY' && error.retryAt === new Date(now + 40_000).toISOString());
  }
  assert.throws(() => admit('client-0'), error => error instanceof ServiceError && error.code === 'PROVIDER_BUSY' && error.retryAt === new Date(now + 10_000).toISOString());
  now += 10_000;
  admit('client-0');
  assert.throws(() => admit('new-client'), error => error instanceof ServiceError && error.code === 'PROVIDER_BUSY' && error.retryAt === new Date(now + 30_000).toISOString());
  now += 30_000;
  exhaust('new-client');
  exhaust('client-999');
  admit('client-0');
...
test('assembled HTTP and provider service charge shared work once and keep cached search/detail available at quota', { timeout: 10_000 }, async t => {
  ...
  const start = (route?: string, options: RequestInput = {}) => app.start(route, { ...options, headers: caddyHeaders(1) });
