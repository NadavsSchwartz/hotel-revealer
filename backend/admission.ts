...
function canonicalIp(value: unknown) {
...
export function clientKey(req: Pick<Request, 'headers' | 'socket'>, clientIdentity = 'socket') {
...
export function createClientLimiter({ now = Date.now } = {}) {
  const clients = new Map<string, { tokens: number; updatedAt: number; expiresAt: number }>();
  const burst = 4;
  ...
  return (key: string) => {
    ...
    if (!bucket) {
      ...
      bucket = { tokens: burst, updatedAt: current, expiresAt: current };
    }
...
export function createHotelAdmission({ clientIdentity = 'socket', now }: { clientIdentity?: string; now?: () => number } = {}): HotelRequestHandler {
  ...
  let operations = 0;
  const clientOperations = new Map<string, number>();
  return (req, res, next) => {
