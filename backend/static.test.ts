...
import { compressBuild } from '../scripts/compress-build.mts';
...
test('production static compression preserves representation negotiation and static-file contracts', async (t) => {
  ...
  function productionApp(frontendDirectory: string, logger: ProviderLogger | null = null) {
    const previousEnvironment = process.env.NODE_ENV;
    try {
      process.env.NODE_ENV = 'production';
      return createApp({ logger, service: {
        search: async () => assert.fail('static request called search'),
        detail: async () => assert.fail('static request called detail'),
      }, frontendDirectory });
    } finally {
  ...
  function request(url: string, headers: http.OutgoingHttpHeaders = {}, method = 'GET', requestPort = port) {
    return new Promise<{ status: number | undefined; headers: http.IncomingHttpHeaders; body: Buffer }>((resolve, reject) => {
      const request = http.request({ host: '127.0.0.1', port: requestPort, path: url, headers, method }, (response) => {
        const chunks: Buffer[] = [];
  ...
  await t.test('identity, Brotli and gzip responses preserve content and MIME type', async () => {
    for (const encoding of [undefined, 'identity', 'br', 'gzip']) {
      ...
      assert.equal(response.headers['content-encoding'], (encoding === 'br' || encoding === 'gzip') ? encoding : undefined);
    ...
    assert.ok(css.headers['content-type']);
  ...
  await t.test('a missing production document fails readiness and logs safe server errors', async (t) => {
    ...
    const missing = productionApp(path.join(directory, 'missing'), { error: entry => { assert.ok(isRecord(entry)); errors.push(entry); } }).listen(0, '127.0.0.1');
    await once(missing, 'listening');
    const missingAddress = missing.address();
    assert.ok(missingAddress && typeof missingAddress !== 'string');
    t.after(() => new Promise((resolve) => { missing.close(resolve); missing.closeAllConnections(); }));
    for (const url of ['/', '/results', '/index.html', '/health']) {
      const response = await request(url, { Accept: 'text/html' }, 'GET', missingAddress.port);
      assert.equal(response.status, 500);
      const body: unknown = JSON.parse(response.body.toString());
    ...
    assert.equal((await request('/assets/missing.js', {}, 'GET', missingAddress.port)).status, 404);
    assert.equal((await request('/api/unknown', {}, 'GET', missingAddress.port)).status, 404);
    const healthy = await request('/health');
    assert.equal(healthy.status, 200);
    assert.deepEqual(JSON.parse(healthy.body.toString()), { status: 'ok', provider: { available: false } });
  });
});
