...
import { getDestination, resolveLegacyCity, searchDestinations } from './index.ts';
...
test('worldwide source includes cities beyond a US or popular-city shortlist', () => {
  const metadata: unknown = JSON.parse(readFileSync(new URL('../../data/destinations-source.json', import.meta.url), 'utf8'));
  assert.ok(isRecord(metadata));
  assert.ok(typeof metadata.catalogCount === 'number' && metadata.catalogCount > 50000);
  assert.ok(typeof metadata.countryCount === 'number' && metadata.countryCount > 200);
...
test('all original city+state URLs retain an exact geographic identity', () => {
  ...
  const bridge: unknown = JSON.parse(readFileSync(new URL('../../data/destinations-legacy-public.json', import.meta.url), 'utf8'));
  assert.ok(Array.isArray(bridge));
  for (const row of bridge) {
    assert.ok(Array.isArray(row) && row.length === 3 && row.every((value: unknown) => typeof value === 'string'));
    const [originalLabel, id, label]: unknown[] = row;
