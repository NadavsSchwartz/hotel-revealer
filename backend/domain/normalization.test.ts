...
import { normalizeListings, normalizeQuote, safeHandoffUrl, safeImageUrl, deduplicateHotels, deduplicateOffers } from './index.ts';
...
test('sparse amenity arrays remain unknown rather than inventing a known code', () => {
  const codes = new Array(1);
...
test('display text and image URLs are bounded before fields can be repeated across candidates', () => {
  ...
  assert.ok(result.hotels[0].neighborhoodName);
...
test('oversized amenity lists and codes become unknown instead of falsely complete truncated evidence', () => {
  ...
  assert.ok(result.hotels[0].amenities);
