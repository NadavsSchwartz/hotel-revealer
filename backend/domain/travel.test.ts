...
import { addCalendarDays, localToday, nightCount, validateTripFields } from '../../shared/travel.ts';
import { contextFromSearch, contextKey, searchUrl, validateContext } from '../../frontend/src/traveler/context.ts';
import { validateSearch } from './validation.ts';
...
test('trip validation distinguishes valid fields from unchanged invalid drafts', () => {
  const valid = validateTripFields(trip, now);
  assert.equal(valid.valid, true);
  assert.deepEqual(valid.errors, {});
  const invalid = validateTripFields({ ...trip, rooms: '2', childrenAges: [null] }, now);
  assert.equal(invalid.valid, false);
  assert.equal(invalid.context.rooms, '2');
  assert.deepEqual(invalid.context.childrenAges, [null]);
  assert.deepEqual(invalid.errorCodes, { rooms: 'INVALID_ROOMS', childrenAges: 'INVALID_CHILD_AGE' });
});
