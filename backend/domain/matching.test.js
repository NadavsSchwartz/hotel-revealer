...
import { matchListings, matchObservations, MatchLimitError, MAX_MATCH_COMPARISONS, MAX_MATCH_CANDIDATES } from './index.js';
...
const resolved = (offers, hotels, options) => matchListings(offers, hotels, options).offers[0];
...
test('zero, one, and multiple hotel identities resolve without selecting an arbitrary match', () => {
  for (const [hotels, resolution, expectedIds] of [
    ...
    if (result.candidates.length) {
      assert.equal(result.candidates[0].tier, undefined);
