...
export default function PropertyPhotos({ images, name }: { images: string[]; name: string }) {
  ...
  const recordFailure = (image: string) => setFailedImages((failed) => (
    failed.includes(image) ? failed : [...failed, image]
  ));
  const move = (offset: number) => {
    if (activeIndex !== null) setSelectedIndex(Math.max(0, Math.min(images.length - 1, activeIndex + offset)));
  };
  const openPhoto = (index: number, event: MouseEvent<HTMLButtonElement>) => {
