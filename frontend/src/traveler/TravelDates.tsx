...
const fullDate = (date: Date) => fullDateFormat.format(date);
const shortDate = (date: Date) => shortDateFormat.format(date);
...
export interface TravelDatesHandle { focus: (field: DateField) => void }
interface TravelDatesProps { trip: TripDraft; errors: TripErrors; onChange: TripChange }

const TravelDates = forwardRef<TravelDatesHandle, TravelDatesProps>(function TravelDates({ trip, errors, onChange }, ref) {
  const checkInRef = useRef<HTMLInputElement>(null);
  const checkOutRef = useRef<HTMLInputElement>(null);
  const popup = useRef<HTMLDivElement>(null);
  const focusAfterSelection = useRef<DateField | null>(null);
  ...
  const inWindow = (value: string) => isCalendarDate(value) && value >= today && value <= latest;
  ...
  useLayoutEffect(() => {
    ...
    const place = (event?: Event) => {
      if (!popup.current || (event?.target instanceof Node && popup.current.contains(event.target))) return;
  ...
  function disabledDate(date: Date, field: DateField) {
  ...
  function focusDay() {
    const day = popup.current?.querySelector<HTMLButtonElement>('.rdp-day_button[tabindex="0"]:not(:disabled):not([aria-disabled="true"])')
      || popup.current?.querySelector<HTMLButtonElement>('.rdp-day_button:not(:disabled):not([aria-disabled="true"])');
    (day || popup.current?.querySelector<HTMLButtonElement>('button:not(:disabled):not([aria-disabled="true"])'))?.focus();
  }

  function triggerKeyDown(event: ReactKeyboardEvent<HTMLInputElement>, field: DateField) {
  ...
  function popupKeyDown(event: ReactKeyboardEvent<HTMLDivElement>) {
    if (event.key !== 'Tab' || !popup.current) return;
    const targets = [...popup.current.querySelectorAll<HTMLButtonElement>('button:not(:disabled):not([aria-disabled="true"])')].filter(element => element.tabIndex !== -1);
  ...
  function selectDate(field: DateField, date: Date | undefined) {
