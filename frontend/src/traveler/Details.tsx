...
import PropertyPhotos from './PropertyPhotos.tsx';
...
import { detailKey, loadDetail, selectDetailView, selectSearchCooldown } from './state.ts';
import {
  ErrorNotice,
  ProviderLink,
  Quote,
...
function PropertyAmenities({ amenities }: { amenities: (string | { name?: string } | null)[] }) {
  const [expanded, setExpanded] = useState(false);
  const names = [...new Set(amenities.map(item => typeof item === 'string' ? item : item?.name)
    .filter((name): name is string => typeof name === 'string' && Boolean(name.trim())).map(name => name.trim().replaceAll('_', ' ')))];
...
export default function Details() {
  ...
  const request = useAppSelector((state) => state.detail);
  const search = useAppSelector((state) => state.searches[contextKey(requestedContext)]);
  const cooldown = useAppSelector((state) => selectSearchCooldown(state, contextKey(requestedContext)));
  ...
  const priceUpdateFailed = offer && !bindingRejected && !(error && 'retryAt' in error && error.retryAt) &&
  ...
  useEffect(() => {
    ...
    void navigate(`/deal?${next}`, { replace: true, state: location.state });
  }, [data, search, input.cityName, context.cityName, location.search, location.state, navigate]);

  useEffect(() => {
    if (valid && validation.valid) void dispatch(loadDetail(data?.context || search?.context || validation.context, offerId, hotelId));
  ...
  if (mapsUrl && mapsUrl.length > 2048) mapsUrl = null;
  ...
  const retry = () => {
    if (!loading && !coolingDown && !bindingRejected && validation.valid) void dispatch(loadDetail(data?.context || search?.context || validation.context, offerId, hotelId));
  };
