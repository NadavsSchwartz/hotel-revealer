...
const Travelers = forwardRef(function Travelers({ trip, errors, onChange }, ref) {
  ...
  const [panelLayout, setPanelLayout] = useState({ placement: 'bottom', maxHeight: 620 });
  ...
  useEffect(() => {
    ...
    const onScroll = (event) => {
      if (!panel.current?.contains(event.target)) positionPanel();
    };
  ...
  function focusPanel() {
    ...
    const ageIndex = ages.findIndex((age) => !Number.isInteger(age) || age < TRAVEL_LIMITS.minChildAge || age > TRAVEL_LIMITS.maxChildAge);
  ...
  function panelKeyDown(event) {
    ...
    if (event.key === 'Tab') {
      const targets = [...panel.current.querySelectorAll('button:not([disabled]), input:not([disabled]), select:not([disabled])')].filter((element) => element.tabIndex !== -1);
            ...
            {ages.map((age, index) => (
              <div className="child-age-field" key={index}>
                ...
                <div className="child-age-select">
                  <select
                    id={`child-age-${index}`}
                    value={Number.isInteger(age) && age >= TRAVEL_LIMITS.minChildAge && age <= TRAVEL_LIMITS.maxChildAge ? age : ''}
                    ...
                    aria-invalid={Boolean(errors.childrenAges && (!Number.isInteger(age) || age < TRAVEL_LIMITS.minChildAge || age > TRAVEL_LIMITS.maxChildAge))}
