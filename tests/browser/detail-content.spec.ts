...
import type { DetailResponse } from '../../shared/contracts.ts';
...
function populated() {
  ...
  present(data.candidate).hotelId = '48700';
  present(data.candidate).name = 'Harbor & Pine';
  data.details = { ...present(data.details), description: null, address: '5 Main Street #7, Las Vegas, NV', images: Array.from({ length: 8 }, (_, index) => imageUrl(index + 1)) };
  return data;
}
...
test('photo browsing loads on demand, handles a failed image and restores keyboard focus', async ({ page }, testInfo) => {
  ...
  await expect.poll(() => viewer.getByRole('img', { name: /property photograph/ }).evaluate(img => img instanceof HTMLImageElement && img.complete && img.naturalWidth > 0)).toBe(true);
...
test('location and reviews link to the named property while amenities retain their qualifiers', async ({ page }, testInfo) => {
  const data = populated();
  present(data.details).images = [];
  present(data.details).amenities = ['Ironing services', 'Free Wi-Fi', 'Valet parking (charges may apply)', 'Outdoor pool', 'Breakfast available', 'Wheelchair accessible', 'Fitness center', 'Air conditioning', 'Restaurant', 'Luggage storage'];
  let calls = 0;
  await page.route('**/api/v1/deal', route => { calls++; return route.fulfill({ json: data }); });
  await page.goto(detailPath(data));
  const maps = page.getByRole('link', { name: /Open in Google Maps/ });
  const href = new URL(present(await maps.getAttribute('href')));
  expect(href.origin + href.pathname).toBe('https://www.google.com/maps/search/');
  expect(href.searchParams.get('api')).toBe('1');
  expect(href.searchParams.get('query')).toBe(`${present(data.candidate).name}, ${present(data.details).address}`);
...
test('a retained gallery survives a smaller photo response and disappears with withdrawn hotel inference', async ({ page }) => {
  ...
  await page.route('**/api/v1/deal', async route => {
    ...
    if (calls === 2) {
      await pause();
      return route.fulfill({ json: { ...data, offer: { ...data.offer, quoteExpiresAt: new Date(now + 120000).toISOString() }, details: { ...present(data.details), images: [imageUrl('new-one'), imageUrl('new-two')] } } });
    }
    ...
    if (calls === 3 || calls === 4) return route.fulfill({ json: { ...data,
      ...
      details: { ...present(data.details), images: calls === 3 ? [] : present(data.details).images.slice(0, 2) } } });
