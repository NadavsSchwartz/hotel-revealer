...
const selectCurrency = page => page.getByRole('combobox', { name: 'Currency', exact: true });
...
test('header preferences stay reachable and currency preserves the home draft and reload choice', async ({ page }, testInfo) => {
  ...
  for (const width of [1440, 390, 320]) {
    ...
    const currencyBox = await selectCurrency(page).boundingBox();
...
test('currency requests new prices, updates every quote and handoff, and follows browser history', async ({ page }, testInfo) => {
  ...
  const details = [];
  let finishEuro;
  const euroReady = new Promise(resolve => { finishEuro = resolve; });
  ...
  await page.route('**/api/v1/deal', route => {
    ...
    data.details.retailQuote = { ...data.offer.quote, nightlyCents: 15900, stayCents: 31800, totalCents: 35600 };
    return route.fulfill({ json: data });
  });
