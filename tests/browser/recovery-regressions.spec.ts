import type { Offer } from '../../shared/contracts.ts';
...
const detailPath = `/deal?${queryParams({ ...context, offerId: 'offer-one', hotelId: 'hotel-one' })}`;

function populatedDetail(now: number) {
  ...
  present(data.details).images = ['https://images.priceline.com/synthetic-recovery/property.webp'];
  return data;
}
...
test('a successful fallback envelope after a failed refresh retains property information and original handoff', async ({ page }) => {
  ...
  try {
    ...
    await expect(heading).toBeVisible();
    await expect(photo).toBeVisible();
    await expect(handoff).toHaveAttribute('href', present(data.offer.handoffUrl));
  } finally { release(); }
...
test('browser Forward withdraws a contradicted hotel while preserving independent original-offer pricing', async ({ page }) => {
  ...
  refreshed.offers[0].candidates = [{ ...present(refreshed.offers[0].candidates[0]), hotelId: 'hotel-two', name: 'Desert House' }];
...
test('retained detail data honors cooldown across every price refresh and return-to-results search', async ({ page }) => {
  ...
  await page.clock.install({ time: now });
  const data = populatedDetail(now);
  present(data.details).retailQuote = { nightlyCents: 15900, stayCents: 31800, currency: 'USD', taxesFees: 'included' };
...
test('offer-only requests retry after connectivity loss and a newly matched response links to a separate hotel view', async ({ page }) => {
  ...
  const unresolved: Offer = { ...offer, resolution: { status: 'unresolved', reason: 'no_match' }, candidates: [] };
...
test('opening Priceline and returning after expiry preserves trip and handoff while withdrawing the total', async ({ page }) => {
  ...
  const path = `/deal?${queryParams({ ...context, offerId: detail.offer.offerId })}`;
