...
import type { ReferenceRow } from './original.mjs';
import { isValidOffer, isValidHotel, matchObservations as matchRefactored, MAX_MATCH_COMPARISONS } from '../../backend/domain/matching.ts';
import { matchListings } from '../../backend/domain/matching.ts';

const hotel = (fields = {}) => ({
  hotelId: 101, name: 'Example Hotel', starRating: 4, overallGuestRating: 8.4, totalReviewCount: 1254,
...
function expectPairs(offers: readonly ReferenceRow[], hotels: readonly ReferenceRow[], expected: readonly unknown[]) {
  assert.deepEqual(matchOriginal(offers, hotels), expected);
  assert.deepEqual(matchRefactored(offers, hotels).pairs, expected);
}

function applicationPairs(offers: readonly unknown[], hotels: readonly unknown[]) {
...
test('public single-hotel resolution preserves raw matching without exposing ambiguous or conflicting identities', () => {
  for (const { rows, original, current } of [
    ...
    const offers = [offer()];
    assert.deepEqual(matchOriginal(offers, rows).map(([, id]) => id), original);
    assert.deepEqual(applicationPairs(offers, rows).map(([, id]) => id), current);
  }
});
...
test('omitting the synthetic source hotel can leave a unique compatible neighbor', () => {
  ...
  // proof of identity. Record the limitation without introducing a new heuristic.
});
