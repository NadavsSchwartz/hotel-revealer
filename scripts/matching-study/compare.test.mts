...
import { isRecord } from '../../backend/domain/validation.ts';
...
test('saved-capture replay reports numeric and padded offer IDs without changing raw parity', async () => {
  ...
  try {
    for (const offerId of [123, '  offer-a  ']) {
      ...
      const report: unknown = JSON.parse(execFileSync(process.execPath,
        [fileURLToPath(new URL('./compare.mjs', import.meta.url)), filename], { encoding: 'utf8' }));
      assert.ok(isRecord(report) && isRecord(report.parity) && isRecord(report.parity.rawOriginal) && isRecord(report.currentMatcher));
